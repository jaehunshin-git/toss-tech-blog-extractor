...
class TossUrlCollector:
    ...
    posts_api_url = "https://api-public.toss.im/api-public/v3/ipd-thor/api/v1/workspaces/15/posts"
    api_page_size = 500
    ...
    def normalize_article_url(href: str | None) -> str | None:
        ...
        return url if len(url) > len("https://toss.tech/article/") and "toss.im" not in url else None
    ...
    def find_last_page_from_pagination(self) -> int | None:
        ...
        try:
            ...
            page_numbers = [
                ...
                for item in BeautifulSoup(response.text, "html.parser").select(PAGINATION_SELECTOR)
    ...
    async def extract_urls_from_page(self, session, page_num: int) -> tuple[int, list[str] | None, int]:
        url = self.base_url.format(page_num)
        try:
            logger.info("Crawling page %s: %s", page_num, url)
            async with session.get(url) as response:
                if response.status != 200:
                    logger.warning("Page %s returned status code: %s", page_num, response.status)
    ...
    async def crawl_pages_parallel(self, max_page: int, max_concurrent: int = 15) -> list[str]:
        start_time = time.time()
        async with create_async_session(max_connections=max_concurrent, timeout_seconds=30) as session:
            results = await asyncio.gather(
                *(self.extract_urls_from_page(session, page) for page in range(1, max_page + 1)),
        ...
        logger.info("Parallel crawling completed in %.2f seconds", time.time() - start_time)
        logger.info("Successfully crawled %s out of %s pages", successful_pages, max_page)
    ...
    async def crawl_pages_until_empty(self, max_pages: int = 100) -> list[str]:
        ...
        all_urls: set[str] = set()
        async with create_async_session(max_connections=5, timeout_seconds=30) as session:
            ...
            while page <= max_pages:
                try:
                    async with session.get(
                        ...
                        if response.status != 200:
                            logger.warning("Posts API returned HTTP %s", response.status)
                            return None
                ...
                if not success.get("next"):
                    ...
                    logger.info("Posts API collected %s unique URLs", len(self.all_urls))
                    return self.all_urls
    ...
    def save_all_urls(self, filename: str | None = None) -> Path:
        ...
        filename = filename or f"toss_url_{datetime.now():%m%d}.txt"
